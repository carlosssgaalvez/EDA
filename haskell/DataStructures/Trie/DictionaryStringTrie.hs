...
-- Nothing y Just relacionado con Empty
...
childOf c (Node _ dic) = toTrie (D.valueOf c dic)
...
update (Node x dic) c child = Node x (D.insert c child dic)

-- | = Exercise i - insert
insert :: String -> a -> Trie a -> Trie a
insert [] v Empty = Node (Just v) D.empty
insert [] v (Node x dic) = Node (Just v) dic
